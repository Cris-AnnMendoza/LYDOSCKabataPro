<?php
/**
 * Secure file viewer for uploaded scholarship documents.
...
// Load document record from scholarship_documents table
$stmt = $pdo->prepare('SELECT * FROM scholarship_documents WHERE id = ? LIMIT 1');
$stmt->execute([$docId]);
$doc = $stmt->fetch();
...
$possiblePaths = [
    __DIR__ . '/../shared/uploads/scholarship/' . $filename,
    __DIR__ . '/../../shared/uploads/scholarship/' . $filename,
    __DIR__ . '/../uploads/scholarship/' . $filename,
];
